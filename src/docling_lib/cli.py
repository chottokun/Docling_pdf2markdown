...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Extract markdown, figures, and tables from a PDF or Office file with high accuracy."
    )
    parser.add_argument(
        "input_file",
        type=Path,
    ...
    if result_path:
        logger.info(f"Workflow completed successfully! Output saved in {output_dir}")
        return 0
    else:
