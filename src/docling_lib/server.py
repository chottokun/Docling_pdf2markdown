...
async def convert_file(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    # The converter saves it as 'extracted_document.md'.
    markdown_files = list(output_dir_path.glob("**/extracted_document.md"))
    if not markdown_files:
        logger.error(f"Conversion succeeded, but the output markdown file is missing.")
        raise HTTPException(status_code=500, detail="Conversion failed to produce an output file: output markdown file is missing.")
