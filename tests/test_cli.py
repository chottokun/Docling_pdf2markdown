...
def test_main_office_input_happy_path(mock_process_pdf, mock_subprocess_run, tmp_path, file_extension):
    ...
    mock_process_pdf.return_value = output_dir / "extracted_document.md"

    # Mock tempfile.TemporaryDirectory to return tmp_path for easier prediction of pdf_file path
    with patch('docling_lib.cli.tempfile.TemporaryDirectory') as mock_temp_dir:
    ...
    assert processed_path.name == f"{input_path.stem}.pdf"
