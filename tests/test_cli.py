...
def test_main_custom_image_scale(mock_process_pdf, tmp_path, pdf_downloader):
    ...
    assert result == 0
    mock_process_pdf.assert_called_once_with(pdf_path, Path("output"), image_scale=1.5)
...
def test_main_processing_fails(mock_process_pdf, tmp_path, caplog, pdf_downloader):
    ...
    mock_process_pdf.assert_called_once_with(pdf_path, tmp_path, image_scale=2.0)
