...
@patch("src.docling_lib.converter.DocumentConverter")
def test_process_pdf_uses_provided_image_scale(
    MockDocumentConverter, tmp_path, pdf_downloader
):
    """
    Given: A custom image_scale value.
    When: process_pdf is called with image_scale.
    Then: The DocumentConverter should be initialized with the provided scale.
    """
    # Arrange
    pdf_path = pdf_downloader("https://arxiv.org/pdf/2406.12430.pdf")
    custom_scale = 3.5
